...
interface RoleManagerProps {
  ...
  onUpdateRole: (role: string, permissions: Partial<RolePermissions>) => Promise<void>
}
...
export const RoleManager: React.FC<RoleManagerProps> = ({
  ...
  const isBaseRole = (role: string) => ["admin", "player", "spectator"].includes(role)
    ...
    return (
      <div className="card">
        ...
        <div style={{ marginTop: "1rem" }}>
          ...
          <ul>
            {Object.entries(rolePermissions[currentUserRole] || {}).map(([permission, hasPermission]) => (
              <li key={permission}>
                {permissionLabels[permission as keyof RolePermissions] || permission}:{" "}
    ...
    <div className="card">
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginTop: "8px" }}>
          {Object.keys(rolePermissions).map((role) => (
            <button
              key={role}
              style={{ padding: "4px 8px", border: selectedRole === role ? "2px solid black" : "1px solid gray" }}
              onClick={() => setSelectedRole(role)}
            >
              <span style={getRoleColor(role)}>{role}</span> {isBaseRole(role) && <small>(Base)</small>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginTop: "1rem" }}>
        {isCreatingRole ? (
          ...
          <button onClick={() => setIsCreatingRole(true)} style={{ width: "100%" }}>
            ➕ Create Custom Role
          </button>
        )}
      </div>
      ...
      {selectedRole && (
        <div style={{ marginTop: "1rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <label>
              Permissions for <span style={getRoleColor(selectedRole)}>{selectedRole}</span>
            </label>
          ...
          <div style={{ maxHeight: "250px", overflowY: "auto", marginTop: "1rem" }}>
            {Object.entries(rolePermissions[selectedRole] || {}).map(([permission, hasPermission]) => (
              <div key={permission} style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handlePermissionChange(permission as keyof RolePermissions, e.target.checked)
                  }
                />
              </div>
            ))}
