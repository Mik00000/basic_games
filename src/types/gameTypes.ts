...
export interface ServerToClientEvents {
  ...
  gameStateUpdated: (data: { state: GameState; canUndo: boolean; canRedo: boolean }) => void
...
export interface ClientToServerEvents {
  createRoom: (data: CreateRoomData, callback: (response: CallbackResponse) => void) => void
  joinRoom: (data: JoinRoomData, callback: (response: CallbackResponse) => void) => void
  ...
  sendChatMessage: (data: { text: string }, callback: (response: CallbackResponse) => void) => void
  undoMove: (callback: (response: CallbackResponse) => void) => void
  redoMove: (callback: (response: CallbackResponse) => void) => void
  updateRoomSettings: (data: Partial<RoomSettings>, callback: (response: CallbackResponse) => void) => void
  updateRolePermissions: (
    data: { role: string; permissions: Partial<RolePermissions> },
    callback: (response: CallbackResponse) => void,
  ) => void
  kickPlayer: (data: { playerId: string }, callback: (response: CallbackResponse) => void) => void
  transferAdmin: (data: { playerId: string }, callback: (response: CallbackResponse) => void) => void
}
